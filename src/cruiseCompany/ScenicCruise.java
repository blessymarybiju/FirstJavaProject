...
public class ScenicCruise extends Cruise implements CruiseCompany {
	...
	public double calculateCruisePrice(int numOfAdult, int childrenAbove5) {
		...
		System.out.println("Scenic Cruise Adults\t\t\t @" + numOfAdult + "\t: $" + adultTicket);
		double kidTicket = kidTicketPrice * childrenAbove5 * numOfDays;
		System.out.println("Scenic Cruise Children Above 5\t \t @" + childrenAbove5 + "\t: $" + kidTicket);
		...
		if (spot != 0) {
			System.out.println("Scenic Cruise Spa\t\t\t @" + spot + "\t: $" + spaRate);
		}
